...
class HTTPError:
    ...
    BOOKING_ALREADY_EXISTS = HTTPErrorItem(
        STATUS_CODE=status.HTTP_409_CONFLICT,
        INFO=ApiInfo.BOOKING_ALREADY_EXISTS,
    )

    EVENT_NOT_BOOKABLE = HTTPErrorItem(
        STATUS_CODE=status.HTTP_409_CONFLICT,
        INFO=ApiInfo.EVENT_NOT_BOOKABLE,
    )

    NOT_ENOUGH_TICKETS = HTTPErrorItem(
        STATUS_CODE=status.HTTP_409_CONFLICT,
        INFO=ApiInfo.NOT_ENOUGH_TICKETS,
    )

    BOOKING_DOES_NOT_EXIST = HTTPErrorItem(
        STATUS_CODE=status.HTTP_404_NOT_FOUND,
        INFO=ApiInfo.BOOKING_DOES_NOT_EXIST,
    )
